...
class FINDHasherp:
	...
	def fromFile(self, filepath):
		...
		try:
			...
			raise e
		return (self.fromImage(img), filepath)
	...
	def fromImage(self,img):
		...
		buffer1 = self.fillFloatLumaFromBufferImage(img)
		...
		return self.findHash256FromFloatLuma(
			buffer1, numRows, numCols, buffer64x64, buffer16x64, buffer16x16
		)

	def fillFloatLumaFromBufferImage(self, img):
		rgb_image = np.array(img.convert("RGB"))
		r = np.array(rgb_image[:,:,0])*self.LUMA_FROM_R_COEFF
		g = np.array(rgb_image[:,:,1])*self.LUMA_FROM_G_COEFF
		b = np.array(rgb_image[:,:,2])*self.LUMA_FROM_B_COEFF
		return (r + g + b).flatten().tolist()
	...
	def findHash256FromFloatLuma(
		...
		self.boxFilter(fullBuffer1,buffer64x64,numRows,numCols,windowSizeAlongRows,windowSizeAlongCols)
	...
	def boxFilter(cls,input,output,rows,cols,rowWin,colWin):
		...
		for i in range(64):
			ini = int(((i + 0.5) * rows) / 64)
			for j in range(64): 
				inj = int(((j + 0.5) * cols) / 64)
				s=0
				xmin=max(0,ini-halfRowWin)
				xmax=min(rows,ini+halfRowWin)
				ymin=max(0,inj-halfColWin)
				ymax=min(cols,inj+halfColWin)
				for k in range(xmin,xmax):
					for l in range(ymin,ymax):
						s+=input[k*rows+l]
				output[i][j]=s/((xmax-xmin)*(ymax-ymin))
